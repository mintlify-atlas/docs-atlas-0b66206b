...
<ParamField path="<leader>tn" type="normal">
  Navigate to next item or jump to next snippet placeholder
  
  ```lua
  ["<leader>tn"] = cmp.mapping(function(fallback)
    if cmp.visible() then
  ...
  **Smart behavior:**
  ...
  - Otherwise: insert normal character
</ParamField>
...
<Tip>
Press `<leader>tn` to expand snippets and jump between placeholders.
</Tip>
...
<Steps>
  ...
  <Step title="Expand snippet">
    Select it and press `<CR>` or `<leader>tn`.
  </Step>
  
  <Step title="Navigate placeholders">
    Press `<leader>tn` to jump between fields.
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Navigation key not working" icon="keyboard">
    **Possible causes:**
    
    1. Conflicting keybindings
    2. Key mapping not configured
    3. Another plugin intercepting keys
    
    **Solutions:**
    
    - Check for keybinding conflicts
    - Verify key mapping in config
